            ...
            <div className="w-full text-gray-900 py-36 h-screen"
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-4 flex items-center justify-center">
                    <div className="lg:w-3/6 xl:w-2/5 md:w-full bg-gray-50 p-8 flex flex-col lg:ml-auto w-full mt-10 lg:mt-0 rounded-md">
                        ...
                        <button onClick={() => addDoc(collection(db, user.auth.currentUser.uid), { uid:user.auth.currentUser.uid, name, totalKm, startKm: totalKm, totalRecords: 0, totalLiters: 0 }).then(() => history.push('/dashboard'))} className="text-white bg-indigo-500 rounded-md border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 text-lg">Submit</button>
                    </div>
                </div>
            </div>}
        </div>
